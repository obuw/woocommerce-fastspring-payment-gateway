    ...
    class WC_FastSpring
    {
        ...
        public function modify_loading_scripts($tag, $handle)
        {
            if ('fastspring' === $handle)
            {
                ...
                return str_replace(' src', ' id="fsc-api" data-storefront="' . $this->get_storefront_path() . '" data-before-requests-callback="fastspringBeforeRequestHandler" data-access-key="' . self::get_setting('access_key') . '" ' . ($debug ? 'data-debug="true" data-test="' . (self::get_setting('testmode') ? 'yes' : 'no') . '" data-version="' . WC_FASTSPRING_VERSION . '" data-data-callback="dataCallbackFunction" data-error-callback="errorCallback"' : '') . ' data-popup-closed="fastspringPopupCloseHandler" data-popup-webhook-received="fastspringPopupCloseHandler" src', $tag);
            }
            return $tag;
        ...
        public static function log($message)
        {
            ...
            if ($settings && $settings['logging'] || defined('WP_DEBUG') && WP_DEBUG)
            {
                if (empty(self::$log))
                {
